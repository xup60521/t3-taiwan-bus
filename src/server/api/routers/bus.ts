...
export const busRouter = createTRPCRouter({
    ...
    getBusEst: publicProcedure.input(z.object({
        ...
        const res = await fetch(`https://tdx.transportdata.tw/api/basic/v2/Bus/EstimatedTimeOfArrival/City/${input.city}/${input.bus}?$select=RouteName,StopName,Direction,NextBusTime,StopStatus,StopSequence,EstimateTime&$filter=RouteName/Zh_tw eq '${input.bus}'&$format=JSON`, {
            headers: {
